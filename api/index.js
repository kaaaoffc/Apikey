...
// --- DATABASE CONFIG (Optimized for Vercel) ---
...
app.get('/tools/qrcode', async (req, res) => {
    ...
    if (!text) return res.send("Masukkan teks! Contoh: /tools/qrcode?text=KaaaOffc");
    try {
        ...
        <body style="background:#020617;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;margin:0;font-family:sans-serif;">
            <div style="background:white;padding:25px;border-radius:40px;box-shadow:0 0 60px rgba(56,189,248,0.3);">
                <img src="${qr}" width="260">
            </div>
            <p style="color:#38bdf8;margin-top:25px;font-weight:900;text-transform:uppercase;letter-spacing:3px;">DATA: ${text}</p>
        </body>`);
    } catch (e) { res.status(500).send("Crash pas bikin QR."); }
});
    ...
    <body class="p-6">
        <div class="max-w-4xl mx-auto">
            <header class="flex justify-between items-center mb-10">
                <h1 class="text-2xl font-black text-sky-400 italic tracking-tighter">KAAA<span class="text-white">OFFC</span></h1>
                <div class="w-10 h-10 rounded-2xl bg-sky-500 flex items-center justify-center font-black text-slate-900 uppercase">K</div>
            </header>
            
            <div class="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-[35px] mb-8">
                <p class="text-[10px] font-black text-emerald-400 mb-4 tracking-[0.3em] text-center uppercase">Jadwal Sholat Jakarta</p>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-6 mb-10 text-center">
                <div class="bg-slate-900/50 p-8 rounded-[40px] border border-white/5">
                    ...
                    <p class="text-[10px] text-slate-500 uppercase mt-2 font-bold">Total API Requests</p>
                </div>
                <div class="bg-slate-900/50 p-8 rounded-[40px] border border-white/5">
                    <p class="text-4xl font-black text-white">48.91 GB</p>
                    <p class="text-[10px] text-slate-500 uppercase mt-2 font-bold">Server RAM Usage</p>
                </div>
                <div class="bg-slate-900/50 p-8 rounded-[40px] border border-white/5">
                    <p class="text-4xl font-black text-emerald-400">ONLINE</p>
                    <p class="text-[10px] text-slate-500 uppercase mt-2 font-bold">System Health</p>
                </div>
            </div>

            <div class="bg-slate-900/50 p-8 rounded-[40px] border border-white/5">
                <h2 class="text-xs font-black text-sky-400 uppercase mb-6 tracking-widest">Live Activity Log</h2>
                <div class="space-y-3">
...
app.get('/login', (req, res) => {
    res.send('<body style="background:#020617;display:flex;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;"><div style="text-align:center;background:#0f172a;padding:60px;border-radius:50px;border:1px solid #38bdf8;"><h1 style="color:#38bdf8;font-weight:900;margin-bottom:40px;">KAAA LOGIN</h1><form action="/auth/login" method="POST"><input name="email" type="email" placeholder="Email" style="padding:15px;border-radius:12px;border:none;margin-bottom:15px;width:100%;"><br><input name="password" type="password" placeholder="Password" style="padding:15px;border-radius:12px;border:none;margin-bottom:25px;width:100%;"><br><button type="submit" style="background:#38bdf8;padding:15px 50px;border:none;border-radius:12px;font-weight:900;cursor:pointer;">GET OTP</button></form></div></body>');
});
...
app.get('/verify', (req, res) => {
    res.send('<body style="background:#020617;display:flex;align-items:center;justify-content:center;height:100vh;"><form action="/auth/verify" method="POST" style="text-align:center;"><h2 style="color:white;font-family:sans-serif;">ENTER OTP</h2><input name="email" type="hidden" value="'+req.query.email+'"><input name="otp" type="text" style="padding:20px;border-radius:15px;text-align:center;font-size:24px;font-weight:900;letter-spacing:10px;"><br><button type="submit" style="background:#38bdf8;padding:15px 40px;margin-top:20px;border-radius:12px;font-weight:900;cursor:pointer;border:none;">VERIFY</button></form></body>');
});
