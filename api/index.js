...
app.use(async (req, res, next) => {
    // Filter biar email/password gak masuk LOG
    ...
    <body class="p-4 md:p-10">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <div class="glass p-5 rounded-3xl border-l-4 border-indigo-500 col-span-2">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                    <div class="glass p-6 rounded-[35px]">
                        ...
                        <p class="text-[11px] text-emerald-400 font-bold mt-1">Status: Premium Verified</p>
                    </div>
                    <div class="glass p-6 rounded-[35px] border border-sky-500/20">
                        <h3 class="text-[10px] font-black text-sky-400 mb-4 uppercase"><i class="fas fa-mosque mr-2"></i> Jadwal Sholat (1301)</h3>
                ...
                <div class="lg:col-span-2 glass p-8 rounded-[40px]">
                    <h3 class="text-[10px] font-black text-sky-400 mb-6 uppercase tracking-widest">HTTP Monitor (Full URL)</h3>
                    <div class="space-y-3">
                        \${logs.map(l => \`
            ...
            async function getSholat() {
                ...
                if(data.status) {
                    ...
                    document.getElementById('sholat-box').innerHTML = \`
                        <div class="flex justify-between"><span>Subuh</span><span class="text-sky-400">\${s.subuh}</span></div>
                        <div class="flex justify-between"><span>Dzuhur</span><span class="text-sky-400">\${s.dzuhur}</span></div>
                        <div class="flex justify-between"><span>Ashar</span><span class="text-sky-400">\${s.ashar}</span></div>
                        <div class="flex justify-between"><span>Maghrib</span><span class="text-sky-400">\${s.maghrib}</span></div>
                        <div class="flex justify-between"><span>Isya</span><span class="text-sky-400">\${s.isya}</span></div>\`;
                }
            }
            getSholat();
        </script>
...
app.get('/search/tiktok', async (req, res) => {
    const { q } = req.query;
    if (!q) return res.json({ status: false, message: "Parameter 'q' diperlukan asu" });
    try {
...
app.get('/ai/gemini', async (req, res) => {
    ...
    if (!text) return res.json({ status: false, message: "Parameter 'text' diperlukan kntl" });
    try {
...
app.get('/information/cuaca', async (req, res) => {
    ...
    if (!kota) return res.json({ status: false, message: "Parameter 'kota' diperlukan" });
    try {
...
app.get('/sholat/1301', async (req, res) => {
    ...
    } catch (e) { res.json({ status: false, error: "Sholat API Error" }); }
});
...
app.post('/auth/verify', async (req, res) => {
    ...
    if (user) { res.cookie('userEmail', email, { maxAge: 86400000 }); res.redirect('/'); } else res.send("Salah!");
});
